# Vu2020 TDVRP benchmark family (Arigliano-generator TD travel times, larger sizes, curated VRP variant)

Satellite benchmark repository of [MAMUT-routing](https://github.com/ANR-MAMUT/MAMUT-routing) (ANR MAMUT, ANR-22-CE22-0016), mounted there as `benchmarks/TDVRP/Vu2020`. Self-contained: instances, canonical arrival-time-function (ATF) sidecars and best-known solutions ship together.
...
This canonical family is the **VRP variant curated by Onyr (2024–2026)**: demands, vehicle capacities, fleet sizes and gaussian service times were generated once by the legacy TDVRPTW-benchmarks pipeline and are pinned here verbatim (recorded per instance in `metadata.legacy_attribute_source`); time windows removed (TDVRP twin). It is therefore **not** comparable with published TD-TSPTW results on the underlying raw files.
